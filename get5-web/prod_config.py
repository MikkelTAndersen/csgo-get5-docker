# copy from --> https://github.com/PhlexPlexico/get5-web/blob/master/instance/prod_config.py.default
##### You must change these before running
SQLALCHEMY_DATABASE_URI = 'mysql://<user!!!>:<pw!!!>@mysqlget5/get5'  # Sqlalchemy database connection info
STEAM_API_KEY = '<steam secret!!!>'  # See https://steamcommunity.com/dev/apikey
SECRET_KEY = '<steam secret!!!>'  # Secret key used for flask cookies
DATABASE_KEY = '<db>'  # Used for encryption on database. MUST BE 16 BYTES.
WEBPANEL_NAME = '<name>' # Used for the title header on the webpage.
